...
async function callClaude({ apiKey, model, systemPrompt, history, userMessage, maxTokens }) {
  const messages = []
...
async function callOpenAI({ apiKey, model, systemPrompt, history, userMessage, maxTokens }) {
...
async function generateReply({ provider, settings, knowledgeItems, history, userMessage }) {
  ...
  if (provider === 'openai') {
    ...
    return callOpenAI({
      ...
      systemPrompt, history, userMessage, maxTokens,
    })
  }
